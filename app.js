...
async function safeJson(response) {
  ...
  try {
    ...
    throw new Error('الخادم أعاد استجابة غير صالحة. حاولي التحديث بعد ثوانٍ.');
  }
}
    ...
    return `
      <div class="record-card bg-slate-50 rounded-2xl p-4 border border-slate-100">
        ...
        <div class="flex gap-2 pt-4 border-t border-slate-200 flex-wrap">
          ${isPending ? `
            <button onclick="approveRequestFull(decodeRecord('${recordJson}'))"
              class="flex-1 p-2 rounded-lg bg-green-600 hover:bg-green-700 text-white text-sm font-semibold transition-colors">
              موافقة + واتساب
            </button>

            <button onclick="rejectRequestFull(decodeRecord('${recordJson}'))"
...
async function approveRequestFull(record) {
  try {
    ...
    showWhatsAppModal({ ...record, status: 'موافق عليه', whatsapp_opened: 1 }, 'approve');
...
async function rejectRequestFull(record) {
  try {
    ...
    await fetchRecords();
    renderAdminRecordsList();
    showWhatsAppModal({ ...record, status: 'مرفوض', whatsapp_opened: 1 }, 'reject');
...
window.approveRequest = async function(id) {
  ...
  await approveRequestFull(record);
};
...
window.rejectRequest = async function(id) {
  ...
  await rejectRequestFull(record);
};
